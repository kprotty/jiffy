...
fn mpsc(c: &mut Criterion) {
    ...
    group.bench_function("loo_ish", |b| {
        b.iter(|| {
            let x = std::sync::Barrier::new(threads + 1);
            let queue = Chan::new(jiffy::loo_ish::Queue::EMPTY);
                ...
                for _ in 0..threads {
                    scope.spawn(|_| {
                        ...
                        for i in 0..messages / threads {
                            queue.send(|c| Result::<(), usize>::Ok(c.send(i))).unwrap();
                        }
                    });
                }
    ...
    //     b.iter(|| {
    //         let x = std::sync::Barrier::new(threads + 1);
    //         let queue = Chan::new(jiffy::unbounded::Queue::new());
            ...
            crossbeam::scope(|scope| {
                for _ in 0..threads {
                    scope.spawn(|_| {
                        x.wait();
                        for i in 0..messages / threads {
                            queue.send(|c| Result::<(), usize>::Ok(c.push(i))).unwrap();
                        }
                    });
                }
